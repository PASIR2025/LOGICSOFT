...
function getGridPx() {
  return Math.max(1, mmToPx(state.snapGridMm || 15));
}
...
function snapSizeToGrid(size) {
  const grid = getGridPx();
  return {
    width: Math.max(grid, roundToGrid(size.width)),
    height: Math.max(grid, roundToGrid(size.height))
  };
}
...
function inferComponentSizeMmFromPixels(widthPx, heightPx) {
  const w = Number(widthPx);
  const h = Number(heightPx);
  if (!(w > 0 && h > 0)) return null;
  // Corel/PNG normalmente exporta cerca de 96 DPI: 1 mm ≈ 3.78 px.
  // Redondeamos al cuadro técnico de 15 mm para que 225x165 px cargue como 60x45 mm.
  const pxPerMmExport = 3.75;
  const gridMm = 15;
  const widthMm = Math.max(gridMm, Math.round((w / pxPerMmExport) / gridMm) * gridMm);
  const heightMm = Math.max(gridMm, Math.round((h / pxPerMmExport) / gridMm) * gridMm);
  return { widthMm, heightMm };
}

function getComponentWidthMmFromJson(json) {
  return firstPositiveNumber(
    json?.widthMm,
    json?.sizeMm?.width,
    json?.size?.widthMm,
    json?.graphics?.widthMm,
    json?.graphics?.sizeMm?.width,
    json?.graphics?.size?.widthMm
  );
}

function getComponentHeightMmFromJson(json) {
  return firstPositiveNumber(
    json?.heightMm,
    json?.sizeMm?.height,
    json?.size?.heightMm,
    json?.graphics?.heightMm,
    json?.graphics?.sizeMm?.height,
    json?.graphics?.size?.heightMm
  );
}

function getComponentSizeMm(def) {
  const widthMm = firstPositiveNumber(
    def?.widthMm,
    def?.sizeMm?.width,
    def?.graphics?.size?.widthMm,
    def?.graphics?.widthMm,
    def?.inferredWidthMm
  );
  const heightMm = firstPositiveNumber(
    def?.heightMm,
    def?.sizeMm?.height,
    def?.graphics?.size?.heightMm,
    def?.graphics?.heightMm,
    def?.inferredHeightMm
  );
  if (widthMm && heightMm) return { widthMm, heightMm };

  // Si el EVC antiguo no trae widthMm/heightMm, NO usamos width/height como mm.
  // Esos valores suelen ser píxeles del PNG. Se convierten a mm estimados y se
  // redondean al módulo de 15 mm para que coincida con la rejilla.
  return inferComponentSizeMmFromPixels(def?.sourceWidth || def?.width, def?.sourceHeight || def?.height);
}

function getTerminalNumber(terminal, ...paths) {
...
function getTerminalNormalizedPosition(terminal, def, sizePx) {
  const sizeMm = getComponentSizeMm(def);
  const widthMm = sizeMm?.widthMm || pxToMm(sizePx.width);
  const heightMm = sizeMm?.heightMm || pxToMm(sizePx.height);

  // Prioridad 1: coordenadas reales en mm generadas desde Corel/generador de pines.
  let xMm = getTerminalNumber(terminal, 'xMm', 'mm.x', 'position.xMm', 'position.mm.x', 'coordinates.xMm');
  let yMm = getTerminalNumber(terminal, 'yMm', 'mm.y', 'position.yMm', 'position.mm.y', 'coordinates.yMm');

  // Compatibilidad: si el terminal declara unidad mm, usar x/y como mm.
  const unit = String(terminal?.unit || terminal?.position?.unit || terminal?.coordinates?.unit || '').toLowerCase();
  if ((xMm === null || yMm === null) && unit === 'mm') {
    xMm = xMm ?? getTerminalNumber(terminal, 'x', 'position.x', 'coordinates.x');
    yMm = yMm ?? getTerminalNumber(terminal, 'y', 'position.y', 'coordinates.y');
  }

  // Muchos generadores guardan solo x/y. Si esos valores caben dentro del tamaño
  // real del componente, los tratamos como milímetros.
  if ((xMm === null || yMm === null) && widthMm > 0 && heightMm > 0) {
    const rawX = getTerminalNumber(terminal, 'x', 'position.x', 'coordinates.x');
    const rawY = getTerminalNumber(terminal, 'y', 'position.y', 'coordinates.y');
    if (rawX !== null && rawY !== null && rawX >= 0 && rawY >= 0 && rawX <= widthMm && rawY <= heightMm) {
      xMm = rawX;
      yMm = rawY;
    }
  }
  ...
  if (xMm !== null && yMm !== null && widthMm > 0 && heightMm > 0) {
    return {
      nx: clamp(xMm / widthMm, 0, 1),
      ny: clamp(yMm / heightMm, 0, 1),
      xMm,
      yMm
    };
  }

  // Prioridad 2: coordenadas en px dentro del SVG/PNG original.
  let xPx = getTerminalNumber(terminal, 'xPx', 'px.x', 'position.xPx', 'position.px.x');
  let yPx = getTerminalNumber(terminal, 'yPx', 'px.y', 'position.yPx', 'position.px.y');
  const sourceW = firstPositiveNumber(def?.sourceWidth, def?.naturalWidth, def?.graphicsWidth, def?.width);
  const sourceH = firstPositiveNumber(def?.sourceHeight, def?.naturalHeight, def?.graphicsHeight, def?.height);
  if (xPx !== null && yPx !== null && sourceW > 0 && sourceH > 0) {
    return { nx: clamp(xPx / sourceW, 0, 1), ny: clamp(yPx / sourceH, 0, 1) };
  }

  // Prioridad 3: posición normalizada 0..1.
  const nx = getTerminalNumber(terminal, 'nx', 'position.nx', 'normalized.x', 'position.normalized.x');
  const ny = getTerminalNumber(terminal, 'ny', 'position.ny', 'normalized.y', 'position.normalized.y');
  return {
    nx: clamp(nx ?? 0.5, 0, 1),
    ny: clamp(ny ?? 0.5, 0, 1)
  };
...
function applyBoardSize(options = {}) {
  ...
  if (boardBackground) {
    ...
    boardBackground.style.backgroundSize = `${gridPx}px ${gridPx}px, ${gridPx}px ${gridPx}px, ${majorGridPx}px ${majorGridPx}px, ${majorGridPx}px ${majorGridPx}px, 100% 100%`;
    boardBackground.dataset.size = `Tablero ${widthMm} mm x ${heightMm} mm · Cuadro ${state.snapGridMm || 15}x${state.snapGridMm || 15} mm`;
  }
...
function showMultiInspector() {
    ...
    <div class="inspector-scroll">
      ...
      <details class="inspector-section" open>
        ...
        <div class="inspector-section-body">
          ...
          <button id="bulkDuplicateGroupBtn" type="button">Duplicar grupo/selección</button>
  ...
  $('#bulkDuplicateGroupBtn', inspectorContent)?.addEventListener('click', () => keepInspectorSectionState(() => duplicateSelectedGroup()));
...
function getExpandedSelectionForGroupCopy(baseComponent = null) {
  const componentMap = new Map();
  const wireMap = new Map();
  const selectedGroups = new Set();

  const addComponentToCopy = el => {
    if (!el) return;
    componentMap.set(el.dataset.id, el);
    if (el.dataset.groupId) selectedGroups.add(el.dataset.groupId);
  };

  if (baseComponent) addComponentToCopy(baseComponent);
  getSelectedComponents().forEach(addComponentToCopy);
  getSelectedWireIds().forEach(id => {
    const wire = state.wires.find(item => item.id === id);
    if (wire) {
      wireMap.set(wire.id, wire);
      if (wire.groupId) selectedGroups.add(wire.groupId);
    }
  });

  selectedGroups.forEach(groupId => {
    $$('.component').forEach(el => {
      if (el.dataset.groupId === groupId) componentMap.set(el.dataset.id, el);
    });
    state.wires.forEach(wire => {
      if (wire.groupId === groupId) wireMap.set(wire.id, wire);
    });
  });

  return {
    components: [...componentMap.values()],
    wires: [...wireMap.values()]
  };
}

function duplicateSelectedGroup(baseComponent = null) {
  const selection = getExpandedSelectionForGroupCopy(baseComponent);
  if (selection.components.length + selection.wires.length < 1) {
    setStatus('No hay grupo o selección para duplicar', 'warning');
    return;
  }

  recordHistory();
  state.groupId = (Number(state.groupId) || 0) + 1;
  const newGroupId = 'grp_' + state.groupId;
  const offset = 32;
  const componentIdMap = new Map();
  const clones = [];

  selection.components.forEach(el => {
    const def = state.libraryMap.get(el.dataset.typeId);
    if (!def) return;
    const clone = addComponent(def, {
      x: (parseFloat(el.style.left) || 0) + offset,
      y: (parseFloat(el.style.top) || 0) + offset,
      name: el.dataset.name || '',
      ref: undefined,
      labelOffsetX: el.dataset.labelOffsetX || 0,
      labelOffsetY: el.dataset.labelOffsetY || 0,
    ...
    if (clone) {
      componentIdMap.set(el.dataset.id, clone.dataset.id);
      clones.push(clone);
    }
  });

  selection.wires.forEach(wire => {
    const fromId = componentIdMap.get(wire.from?.componentId);
    const toId = componentIdMap.get(wire.to?.componentId);
    if (!fromId || !toId) return;
    state.wireId += 1;
    state.wires.push({
      ...JSON.parse(JSON.stringify(wire)),
      id: 'wire_' + state.wireId,
      from: { ...wire.from, componentId: fromId },
      to: { ...wire.to, componentId: toId },
      waypoints: (wire.waypoints || []).map(point => ({ x: point.x + offset, y: point.y + offset })),
      groupId: newGroupId
    });
  });

  clearSelection();
  clones.forEach(el => selectComponent(el, true));
  state.selectedWires.clear();
  state.wires.forEach(wire => { if (wire.groupId === newGroupId) state.selectedWires.add(wire.id); });
  if (getTotalSelectionCount() > 1) showMultiInspector();
  else if (clones[0]) showComponentInspector(clones[0]);
  drawWires();
  saveState();
  setStatus('Grupo duplicado', 'success');
}
...
function showComponentInspector(el) {
  ...
  inspectorContent.innerHTML = `
    ...
    <details class="inspector-section">
      ...
      <div class="inspector-section-body">
        ...
        <button id="duplicateGroupBtn" type="button">Duplicar grupo</button>
      </div>
    </details>
  ...
  $('#duplicateGroupBtn', inspectorContent)?.addEventListener('click', () => keepInspectorSectionState(() => duplicateSelectedGroup(el)));
  ...
  $('#applyProps', inspectorContent).addEventListener('click', () => {
    ...
    const nextW = roundToGrid(clamp(getPxValueFromDisplay($('#propWidth', inspectorContent).value) || COMPONENT_SIZE.target, 20, 4000));
    const nextH = roundToGrid(clamp(getPxValueFromDisplay($('#propHeight', inspectorContent).value) || COMPONENT_SIZE.target, 20, 4000));
...
function snapCoordinate(value, grid = getGridPx(), threshold = state.snapThreshold) {
  if (!state.snapEnabled) return value;
  const safeGrid = Math.max(1, Number(grid) || getGridPx());
...
function applySmartSnapToComponent(item, proposedX, proposedY) {
  ...
  const threshold = Math.max(4, getGridPx() / 2);
...
function ensureMarqueeElement() {
  let box = $('#marqueeSelection');
  if (!box) {
    box = document.createElement('div');
    box.id = 'marqueeSelection';
...
function getComponentWorldRect(el) {
...
function getWireWorldRect(wire) {
  const points = getWirePoints(wire);
  ...
  const pad = Math.max(8, getWireStroke(wire) + 6);
  return { left: left - pad, top: top - pad, right: right + pad, bottom: bottom + pad, width: right - left + pad * 2, height: bottom - top + pad * 2 };
}

function startMarqueeSelection(event) {
  if (!state.multiSelectMode || state.mode !== 'select' || event.button !== 0 || !isWorkspaceBackgroundTarget(event.target)) return false;
  event.preventDefault();
  event.stopPropagation();
  const rect = workspace.getBoundingClientRect();
  ...
  const box = ensureMarqueeElement();
  box.style.display = 'block';
  box.style.left = startClient.x + 'px';
  box.style.top = startClient.y + 'px';
  box.style.width = '0px';
...
function updateMarqueeSelection(event) {
  ...
  const rect = workspace.getBoundingClientRect();
  const currentClient = { x: event.clientX - rect.left, y: event.clientY - rect.top };
  const currentWorld = clientToWorld(event.clientX, event.clientY);
  const clientRect = normalizeRect(state.marqueeSelection.startClient, currentClient);
  const box = ensureMarqueeElement();
  ...
  state.marqueeSelection.lastClient = currentClient;
  state.marqueeSelection.lastWorld = currentWorld;
  state.marqueeSelection.moved = state.marqueeSelection.moved || clientRect.width > 4 || clientRect.height > 4;
}

function applyMarqueeSelection() {
  const marquee = state.marqueeSelection;
  const box = $('#marqueeSelection');
  if (box) box.style.display = 'none';
  state.marqueeSelection = null;
  if (!marquee || !marquee.moved) return;

  // Evita que el click final del navegador limpie la selección recién creada.
  state.suppressWorkspaceClick = true;
  setTimeout(() => { state.suppressWorkspaceClick = false; }, 80);

  const workspaceRect = workspace.getBoundingClientRect();
  const clientRectLocal = normalizeRect(marquee.startClient, marquee.lastClient);
  const clientRect = {
    left: workspaceRect.left + clientRectLocal.left,
    top: workspaceRect.top + clientRectLocal.top,
    right: workspaceRect.left + clientRectLocal.right,
    bottom: workspaceRect.top + clientRectLocal.bottom,
    width: clientRectLocal.width,
    height: clientRectLocal.height
  };
  ...
  // Selección real por marco usando coordenadas de pantalla. Es más confiable con zoom/pan.
  $$('.component').forEach(el => {
    const rect = el.getBoundingClientRect();
    const compRect = { left: rect.left, top: rect.top, right: rect.right, bottom: rect.bottom, width: rect.width, height: rect.height };
    const center = { x: rect.left + rect.width / 2, y: rect.top + rect.height / 2 };
    if (rectsIntersect(compRect, clientRect) || pointInRect(center, clientRect)) {
      el.classList.add('selected');
      state.selectedComponents.add(el.dataset.id);
    }
  });

  // Para cables: primero intentamos con la caja SVG real; si no existe, usamos la caja en coordenadas del tablero.
  state.wires.forEach(wire => {
    let selected = false;
    const hit = wiresSvg.querySelector(`.wire-hit[data-wire-id="${wire.id}"]`);
    if (hit && hit.getBoundingClientRect) {
      const rect = hit.getBoundingClientRect();
      const wireClientRect = { left: rect.left, top: rect.top, right: rect.right, bottom: rect.bottom, width: rect.width, height: rect.height };
      selected = rectsIntersect(wireClientRect, clientRect);
    }
    if (!selected) {
      const worldRect = getWireWorldRect(wire);
      if (worldRect) {
        const wireClientRect = {
          left: workspaceRect.left + state.panX + worldRect.left * state.zoom,
          top: workspaceRect.top + state.panY + worldRect.top * state.zoom,
          right: workspaceRect.left + state.panX + worldRect.right * state.zoom,
          bottom: workspaceRect.top + state.panY + worldRect.bottom * state.zoom
        };
        wireClientRect.width = wireClientRect.right - wireClientRect.left;
        wireClientRect.height = wireClientRect.bottom - wireClientRect.top;
        selected = rectsIntersect(wireClientRect, clientRect);
      }
    }
    if (selected) {
      state.selectedWires.add(wire.id);
  ...
  if (total > 1) {
    syncSelectionState('multi', 'multi');
    showMultiInspector();
    setStatus(total + ' elemento(s) seleccionados', 'success');
  } else if (state.selectedComponents.size === 1) {
    const el = getSelectedComponents()[0];
    syncSelectionState('component', el.dataset.id);
    showComponentInspector(el);
  } else if (state.selectedWireId) {
    syncSelectionState('wire', state.selectedWireId);
    ...
    syncSelectionState(null, null);
    renderInspectorEmpty();
    setStatus('No se encontraron elementos dentro del marco', 'warning');
  }
  drawWires();
}
...
function normalizeLibraryComponent(component) {
  ...
  const sourceWidth = Number(component.sourceWidth || component.width) || 120;
  const sourceHeight = Number(component.sourceHeight || component.height) || 120;
  const explicitWidthMm = firstPositiveNumber(component.widthMm, component.sizeMm?.width, component.graphics?.size?.widthMm, component.graphics?.widthMm);
  const explicitHeightMm = firstPositiveNumber(component.heightMm, component.sizeMm?.height, component.graphics?.size?.heightMm, component.graphics?.heightMm);
  const inferred = inferComponentSizeMmFromPixels(sourceWidth, sourceHeight);
  const widthMm = explicitWidthMm || inferred?.widthMm || 60;
  const heightMm = explicitHeightMm || inferred?.heightMm || 45;
  return {
    id: component.id,
    ...
    widthMm,
    heightMm,
    inferredWidthMm: widthMm,
    inferredHeightMm: heightMm,
...
async function handleFiles(event) {
  const files = [...event.target.files].filter(file => /\.(evc|zip)$/i.test(file.name));
  if (!files.length) {
    setStatus('No se encontraron archivos .evc o .zip', 'warning');
    event.target.value = '';
    return;
  }
  let imported = 0;
  for (const file of files) {
    const ok = await loadEvcFile(file, false, getLibraryCategoryFromFile(file));
    if (ok) imported += 1;
  }
  renderLibrary();
  setStatus(`Biblioteca cargada: ${imported} componente(s)`, imported ? 'success' : 'warning');
  event.target.value = '';
}

function normalizePasirV2Component(json, fileName, normalSvgText = '', activeSvgText = '', forcedCategory = null) {
  const widthMm = getComponentWidthMmFromJson(json) || firstPositiveNumber(json?.widthMm, json?.sizeMm?.width) || 60;
  const heightMm = getComponentHeightMmFromJson(json) || firstPositiveNumber(json?.heightMm, json?.sizeMm?.height) || 45;
  const normalUrl = normalSvgText
    ? 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(normalSvgText)
    : placeholderComponentSvgDataUrl(widthMm, heightMm, json?.name || fileName);
  const activeUrl = activeSvgText
    ? 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(activeSvgText)
    : normalUrl;

  const ports = Array.isArray(json?.ports) ? json.ports : (Array.isArray(json?.terminals) ? json.terminals : []);
  return {
    id: json?.id || `${fileName}_${Date.now()}`,
    fileName,
    format: json?.format || 'pasir-evc',
    version: json?.version || 2,
    name: json?.name || fileName.replace(/\.(evc|zip)$/i, ''),
    tag: json?.tag || '',
    ...
    sourceWidth: mmToPx(widthMm),
    sourceHeight: mmToPx(heightMm),
    image: normalUrl,
    normalImage: normalUrl,
    activeImage: activeUrl,
    layers: { normal: normalUrl, active: activeUrl },
    terminals: ports.map((p, i) => ({
      id: String(p.id || p.name || (i + 1)),
      label: String(p.label || p.id || p.name || (i + 1)),
      signal: p.signal || p.type || 'Control',
      direction: String(p.direction || p.dir || 'bottom').toLowerCase(),
      xMm: Number(p.xMm ?? p.x ?? p.position?.xMm ?? p.position?.x ?? 0),
      yMm: Number(p.yMm ?? p.y ?? p.position?.yMm ?? p.position?.y ?? 0),
      nx: Number.isFinite(Number(p.nx)) ? Number(p.nx) : undefined,
      ny: Number.isFinite(Number(p.ny)) ? Number(p.ny) : undefined,
      unit: 'mm'
    }))
  };
}

function placeholderComponentSvgDataUrl(widthMm = 60, heightMm = 45, name = 'Componente') {
  const safeName = xmlEscape(String(name || 'Componente'));
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${widthMm}mm" height="${heightMm}mm" viewBox="0 0 ${widthMm} ${heightMm}"><rect x="0.5" y="0.5" width="${Math.max(1, widthMm - 1)}" height="${Math.max(1, heightMm - 1)}" rx="2" fill="#f8fafc" stroke="#2563eb"/><text x="${widthMm / 2}" y="${heightMm / 2}" text-anchor="middle" dominant-baseline="middle" font-family="Arial" font-size="4" fill="#111827">${safeName}</text></svg>`;
  return 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(svg);
}

function findZipFile(zip, names) {
  for (const name of names) {
    const direct = zip.file(name);
    if (direct) return direct;
  }
  const lowerNames = names.map(n => n.toLowerCase());
  const found = Object.values(zip.files).find(entry => !entry.dir && lowerNames.includes(entry.name.split('/').pop().toLowerCase()));
  return found || null;
}

async function loadPasirV2Zip(file, zip, forcedCategory) {
  const manifest = findZipFile(zip, ['componente.evc', 'component.evc', 'pasir.evc']);
  if (!manifest) return null;
  const json = JSON.parse(await manifest.async('string'));
  const normalName = json?.layers?.normal || json?.normalSvg || 'normal.svg';
  const activeName = json?.layers?.active || json?.activeSvg || 'active.svg';
  const normalFile = findZipFile(zip, [normalName, 'normal.svg']);
  const activeFile = findZipFile(zip, [activeName, 'active.svg']);
  const normalSvgText = normalFile ? await normalFile.async('string') : '';
  const activeSvgText = activeFile ? await activeFile.async('string') : normalSvgText;
  return normalizePasirV2Component(json, file.name, normalSvgText, activeSvgText, forcedCategory);
}

async function loadLegacyZip(file, zip, forcedCategory) {
  const manifest = zip.file('component.json');
  const imageFile = zip.file('original.png') || zip.file('preview.png');
  if (!manifest || !imageFile) return null;

  const json = JSON.parse(await manifest.async('string'));
  const base64 = await imageFile.async('base64');
  const id = json.id || file.name;
  const explicitWidthMm = getComponentWidthMmFromJson(json);
  const explicitHeightMm = getComponentHeightMmFromJson(json);
  const rawWidth = firstPositiveNumber(json.graphics?.size?.width, json.width, json.sourceWidth, 120);
  const rawHeight = firstPositiveNumber(json.graphics?.size?.height, json.height, json.sourceHeight, 120);
  const inferredMm = inferComponentSizeMmFromPixels(rawWidth, rawHeight);
  return {
    id,
    fileName: file.name,
    name: json.name || file.name,
    widthMm: explicitWidthMm || inferredMm?.widthMm || 60,
    heightMm: explicitHeightMm || inferredMm?.heightMm || 45,
    inferredWidthMm: inferredMm?.widthMm,
    inferredHeightMm: inferredMm?.heightMm,
    width: rawWidth,
    height: rawHeight,
...
async function loadEvcFile(file, rerender = true, forcedCategory = null) {
  try {
    ...
    // Nuevo formato recomendado: ZIP generado por el generador SVG doble capa.
    // Contiene: componente.evc + normal.svg + active.svg.
    try {
      const zip = await JSZip.loadAsync(file);
      component = await loadPasirV2Zip(file, zip, forcedCategory);
      if (!component) component = await loadLegacyZip(file, zip, forcedCategory);
    } catch (_) {
      // No era ZIP; puede ser un .evc JSON suelto.
    }

    if (!component) {
      const text = await file.text();
      const json = JSON.parse(text);
      component = normalizePasirV2Component(json, file.name, '', '', forcedCategory);
      if (!json.normalSvg && !json.layers?.normal) {
        setStatus('EVC JSON cargado sin SVG: se creó una vista temporal. Exporta ZIP desde el generador para calidad real.', 'warning');
      }
    }

    if (!component) throw new Error('Formato EVC no reconocido');
    upsertLibraryComponent(component, { render: rerender, persist: true });
    return true;
  } catch (error) {
    console.error(error);
    alert('No se pudo leer ' + file.name + '. Usa el ZIP del generador: componente.evc + normal.svg + active.svg.');
    return false;
  }
}
...
function getRecommendedDisplaySize(def) {
  const sizeMm = getComponentSizeMm(def);
  if (sizeMm) {
    // Solución: EVC manda la medida real en mm y la app solo convierte a píxeles.
    // No se usan SIZE_PRESETS ni target/min/max para que Corel y los pines coincidan.
    return snapSizeToGrid({
      width: mmToPx(sizeMm.widthMm),
      height: mmToPx(sizeMm.heightMm)
    });
  }

  // Último respaldo para EVC sin medida técnica.
  const sourceW = Number(def?.width) || COMPONENT_SIZE.target;
  const sourceH = Number(def?.height) || COMPONENT_SIZE.target;
  return snapSizeToGrid({ width: sourceW, height: sourceH });
}
...
function selectLibraryComponentForPlacement(typeId) {
  ...
  closeLibraryDrawer();
  setMode('select');
setStatus(`Área técnica EVC real lista: rejilla ${state.snapGridMm || 15}x${state.snapGridMm || 15} mm · snap exacto activado`, 'success');
...
function addComponent(def, options = {}) {
  ...
  el.style.left = snapCoordinate(options.x ?? 200) + 'px';
  el.style.top = snapCoordinate(options.y ?? 150) + 'px';
  ...
  def.terminals.forEach(terminal => {
    const terminalPos = getTerminalNormalizedPosition(terminal, def, size);
    const x = terminalPos.nx;
    const y = terminalPos.ny;
    ...
    if (Number.isFinite(terminalPos.xMm)) port.dataset.xMm = String(terminalPos.xMm);
    if (Number.isFinite(terminalPos.yMm)) port.dataset.yMm = String(terminalPos.yMm);
...
function startComponentResize(event, el) {
  setStatus('Escalado bloqueado: diseña el tamaño en Corel en múltiplos de 15 mm', 'warning');
  return;
  if (event.button !== 0) return;
  if (el.dataset.locked === 'true') {
    setStatus('Componente bloqueado', 'warning');
...
function drawWires() {
  ...
  state.wires.forEach((wire, wireIndex) => {
    ...
    hitPath.dataset.wireId = wire.id;
...
document.addEventListener('mousemove', event => {
  ...
  if (state.dragContext) {
    ...
    let effectiveDx = dx;
    let effectiveDy = dy;
    const anchor = state.dragContext.origins[0];
    if (anchor) {
      const anchorNext = applySmartSnapToComponent(anchor, anchor.x + dx, anchor.y + dy);
      effectiveDx = anchorNext.x - anchor.x;
      effectiveDy = anchorNext.y - anchor.y;
    }
    state.dragContext.origins.forEach(item => {
      item.el.style.left = (item.x + effectiveDx) + 'px';
      item.el.style.top = (item.y + effectiveDy) + 'px';
    });
    (state.dragContext.wireOrigins || []).forEach(item => {
      item.wire.waypoints = item.waypoints.map(point => ({ x: point.x + effectiveDx, y: point.y + effectiveDy }));
    });
    requestDrawWires();
  }
});
...
document.addEventListener('keydown', event => {
  if (event.key === 'Escape' && state.marqueeSelection) { cancelMarqueeSelection(); return; }
